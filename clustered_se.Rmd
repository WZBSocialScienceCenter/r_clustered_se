...
By minimizing $e = Y - X\beta$, an estimation for our parameters, $\hat\beta$, can be found. [Roberts 2013] shows how the variance of the parameter estimates $V[\hat\beta]$ can be derived which results in the *sandwich estimator*

\begin{equation} 
V[\hat\beta] = (X^TX)^{-1} X^T \boldsymbol\Sigma X (X^TX)^{-1}.
(\#eq:sandwich)
\end{equation} 
...
```{r}
# Empirical Estimating Function diag((Y - X beta)) * X = diag(resid) *  X
# in R same as: residuals(m1) * X
Y <- as.matrix(nlswork$ln_wage)
betahat <- as.matrix(coef(m1))
u <- diag(as.vector(Y - X %*% betahat)) %*% X    # residuals scale each regressor
#u <- estfun(m1)
n_clust <- length(levels(nlswork$idcode))

u_clust <- sapply(1:ncol(X), function(j) {
  # equivalent to: group_by(data.frame(u = u[,j], idcode = nlswork$idcode), idcode) %>% summarise(sum(u))
  tapply(u[,j], nlswork$idcode, sum)
})

meat_clust <- (n_clust / (n_clust-1)) * t(u_clust) %*% u_clust
```

```{r}
sqrt(diag(crossXinv %*% meat_clust %*% crossXinv))
```
